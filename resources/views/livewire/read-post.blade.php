<div class="pt-16 px-2">
    <div class="grid grid-cols-12 gap-4 grid-flow-row">
        <div class="col-span-12  md:col-span-7 owerflow-hidden pb-5 md:col-start-2 " style="height: fit-content;" >
            <div class="h-[40%] mt-2">
                    @if (file_exists(public_path($article->image['indexArray']['small'])))
                           <img src="{{ asset($article->image['indexArray']['large']) }}"alt=""
            ...
            >
                <div class="">
                    <div class="flex gap-2">
                        ...
                        ></i>
                        <p class=" text-black dark:text-white">نویسنده :  {{  $article->arthor->fullName() }}</p>
                    </div>
                </div>
                <div class="">
                    <div class="flex gap-2">
...
>تاریخ :   {{ $persianDate }}</p>
        ...
        <div class="col-span-12  md:col-span-3 z-2 mr-3 " >
            ...
            >
                ...
                @if ($categoryArticles)
                     @foreach ($categoryArticles->articles as $item)
                      <div class="flex gap-2 flex-wrap">
                    <div class="p-1 rounded-md w-full">
                        ...
                        >
                            ...
                            ></div>
                            <a href="{{route("read-post" , $item->id )}}" class=""> 
                                <p class=" text-black dark:text-white mt-2  mr-4"> {{$item->title}}</p>
            ...
            >
                <div class="flex items-center justify-center gap-4">
                    ...
                    <div class="">
                        <p class=" text-black dark:text-white text-md"> {{$article->arthor->fullName() }}  </p>
                    </div>
                </div>
